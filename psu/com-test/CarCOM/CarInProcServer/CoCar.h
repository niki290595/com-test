...
CoCar::CoCar() : m_refCount(0), m_currSpeed(0), m_maxSpeed(0) {
	m_petName = SysAllocString(L"MERS");
	g_objCount++;
}
...
STDMETHODIMP CoCar::DisplayStats() {
	...
	MessageBox(NULL, L"Mers"/*(LPCWSTR)m_currSpeed*/, L"Pet Name", MB_OK | MB_SETFOREGROUND);
	memset(buff, 0, sizeof(buff));
	sprintf_s(buff, "%d", m_maxSpeed);
	MessageBox(NULL, L"45"/*(LPCWSTR)m_currSpeed*/, L"MAX Speed", MB_OK | MB_SETFOREGROUND);
	return S_OK;
}
